        ...
        <form className={`${activeTab === 'add' ? 'block animate-fade-in animate__animated animate__fadeIn' : 'hidden animate-fade-out animate__animated animate__fadeOut'}`}>
          ...
          <div className="text-center">
            ...
            <div className='flex items-center justify-center text-xs h-10 my-4 border-t-1 border-gray-400'>
              {registrationSuccess && <p className="text-green-900 font-semibold font-cocogoose text-[0.65rem] uppercase mt-3">¡Felicidades! Has generado la solicitud de Transferencia con éxito. <br />El envío se realizará en las próximas horas.</p>}
          ...
          {tickets.length > 0 ? (
              ...
              <table className="min-w-full text-center text-sm font-light table-fixed">
                <thead className="font-medium text-gray-900">
                  <tr className="border-b-2 border-slate-400 font-cocogoose font-semibold uppercase text-xs">
                    <th scope="col" className="w-1/5 px-6 py-1">ID</th>
                    <th scope="col" className="w-1/5 px-6 py-1">Wallet</th>
                    <th scope="col" className="w-1/5 px-6 py-1">Valor</th>
                    <th scope="col" className="w-1/5 px-6 py-1">Fecha</th>
                    <th scope="col" className="w-1/5 px-6 py-1">Estado</th>
                  </tr>
                </thead>
                <tbody>
                  {filledTickets.map((obj, index) => (
                    <tr key={index} className="border-b border-slate-300 uppercase text-xs text-gray-600 text-center align-middle h-6">
                      <td className="w-1/5 whitespace-nowrap px-6 py-1 font-bankprinter">{obj.voucher ? obj.voucher : ''}</td>
                      <td className="w-1/5 whitespace-nowrap px-6 py-1 font-bankprinter">{obj.address ? obj.address : ''}</td>
                      <td className="w-1/5 whitespace-nowrap px-6 py-1 font-bankprinter">{obj.amount ? formatNumber(obj.amount) : ''}</td>
                      <td className="w-1/5 whitespace-nowrap px-6 py-1 font-bankprinter">{obj.date ? obj.date : ''}</td>
                      <td className="w-1/5 whitespace-nowrap px-6 py-1 font-bankprinter">{obj.state ? obj.state : ''}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className='w-full h-full flex flex-col justify-start items-center'>
              <span className='text-center text-gray-800 mt-8 text-sm'>
                <p>¡Aun no has realizado ningun retiro!</p>
                <p>Las transacciones dependeran de la red seleccionada.</p>
              </span>
            </div>
          )}
        </section>

        ...
        <div className="absolute bottom-10 w-full px-4">
          {activated ? (
            <p className="h-10 bg-green-700 text-white font-semibold font-cocogoose text-xs rounded-sm py-2 px-4 w-full text-center flex items-center justify-center">
              {t('home.header.auth.message.activated')}
            </p>
          ) : loading ? (
            ...
            <button type="button" onClick={redirectToLogin} className="h-10 bg-gray-800 hover:bg-gray-900 text-white font-semibold rounded-sm py-2 px-4 w-full font-cocogoose text-xs text-center uppercase">
              {t('home.header.auth.message.login')}
            </button>
          )}
        </div>
      </div>
